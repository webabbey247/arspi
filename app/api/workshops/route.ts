...
/** GET /api/workshops — list workshops (admin only) */
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("[GET /api/workshops]", error)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await createWorkshop({ ...input, slug })
    if (!result.success) {
    ...
    console.error("[POST /api/workshops]", error)
    return NextResponse.json({ error: "Something went wrong." }, { status: 500 })
  }
}
